...
class Cell(Element, Layout):
    ...
    def __init__(self, raw:dict=None):
        ...
        self.logis_cells = raw.get('logis_cells', (1,1, 1, 1)) # type: tuple [int]
    ...
    def make_docx(self, table, indexes):
        ...
        start_row, start_column, end_row, end_column = self.logis_cells

        # print("start_row: {}, start_column: {}, end_row: {}, end_column: {}".format(start_row, start_column, end_row, end_column))

        # n_row, n_col = self.merged_cells

        n_row, n_col = start_row, start_column

        i, j = indexes

        docx_cell = table.cell(n_row, n_col)
        # print("text:{}   bbox:{}     merged_cells:{}".format(self.text,self.bbox,self.merged_cells))
        import traceback
        ##update by chen
        # if n_row*n_col!=1:
        #     try:
        ...
        # if (end_row-start_row+1) * (end_column-start_column+1) != 1 and (start_column + n_col <= end_column):
        if (end_row-start_row+1) * (end_column-start_column+1) != 1:

            try:
                print("start_row: {}, start_column: {}, end_row: {}, end_column: {}, n_row: {}, n_col: {}".format(start_row, start_column, end_row, end_column, n_row, n_col))
                # _cell = table.cell(start_row+n_row-1, j+n_col-1)
                _cell = table.cell(end_row, end_column)
                docx_cell.merge(_cell)
            except:
                n_row, n_col=1,1
                _cell = table.cell(i + n_row - 1, start_column + n_col - 1)
                docx_cell.merge(_cell)
                print(traceback.format_exc())
        ...
        # table.columns[start_column].width = Pt(x1-x0)
        # insert contents
