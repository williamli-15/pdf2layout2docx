...
class Layout:
    ...
    def parse(self, **settings):
        ...
        if (not settings['img_to_convert_docx']):
            # parse tables
            self._parse_table(**settings)            #***在line中线找出table表格并转换成TableBlock，其中shapes没有更新

        # parse paragraphs
        self._parse_paragraph(**settings)         #*** 在lines文本行转换成TextBlock，并按顺序排列【包括TableBlock】
